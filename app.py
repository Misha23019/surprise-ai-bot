...
def telegram_webhook():
    ...
    if user_input.startswith("/lang"):
        ...
        if len(parts) == 2 and parts[1] in LANGUAGES:
            ...
            send_message(chat_id, f"✅ Мова змінена на {LANGUAGES[parts[1]]}\n⌚ Введіть поточний час у форматі ГГ:ХХ (наприклад, 09:30), щоб отримувати сюрприз о 10:00", TELEGRAM_TOKEN)
        else:
            langs_list = "\n".join([f"{k} - {v}" for k, v in LANGUAGES.items()])
    ...
    reply = generate_response(chat_id, user_input, lang)
